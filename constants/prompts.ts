/**
 * Centralized prompts for all Gemini AI interactions.
 * Edit these to fine-tune the creative outputs.
 */

/**
 * Prompt for extracting the main object from an image as a binary mask.
 * Used by: extractObjectFromImage()
 */
export const OBJECT_EXTRACTION_PROMPT = `Image Editing Task: Create a high-contrast binary segmentation mask for the main significant foreground object (or group of objects) in this image. The object(s) should be pure WHITE (#FFFFFF) and the background should be pure BLACK (#000000). Ensure the edges are precise and the mask is solid.`;

/**
 * Prompt for transforming an image in the style of René Magritte.
 * Used by: transformToMagritteStyle()
 * 
 * Note: This prompt references 11 Magritte paintings that are passed as
 * additional image inputs. The {orientationDesc} placeholder is replaced
 * at runtime with the actual image orientation.
 */
export const MAGRITTE_TRANSFORM_PROMPT = (orientationDesc: string) => `You are a visual artist channeling René Magritte's surrealist philosophy.
...
/**
 * Prompt for generating witty, surreal captions for images.
 * Used by: generateCaption()
 */
export const CAPTION_GENERATION_PROMPT = `You are a sharp, empathetic caption writer for surrealistic images.
...
- First, in your head, parse the scene: main subjects, odd or funny juxtapositions, any tension between expectation and reality, and clues about place, class, aspiration, or online culture.
...
- Finally, write a single caption that:
    - is at most 25 words
    - uses casual social-media English
